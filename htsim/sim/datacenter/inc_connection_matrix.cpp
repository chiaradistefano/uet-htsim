...
#include "inc_connection_matrix.h"
#include <string.h>
#include <stdio.h>
#include <iostream>
#include "math.h"
...
IncConnectionMatrix::IncConnectionMatrix(uint32_t n)
{
  N = n;
}

void IncConnectionMatrix::setPermutation(uint32_t conn){
  setPermutation(conn,1);
}

void IncConnectionMatrix::addConnection(uint32_t src, uint32_t dest){
...
void IncConnectionMatrix::setPermutation(uint32_t conn, uint32_t rack_size){
    //int is_dest[N];
    uint32_t dest,pos;
...
void IncConnectionMatrix::setPermutation(){
    int is_dest[N];
    uint32_t dest;
...
void IncConnectionMatrix::setStride(uint32_t conns){
...
void IncConnectionMatrix::setPermutationShuffle(uint32_t conns){
...
void IncConnectionMatrix::setPermutationShuffleIncast(uint32_t conns){
    // we want random sources and destinations
    // we'll shuffle all the destinations for all possible sources, then pick the right number of connection afterwards
...
void IncConnectionMatrix::setLocalTraffic(Topology* top){
    for (uint32_t src = 0;src<N;src++){
        connections[src] = new vector<uint32_t>();
...
void IncConnectionMatrix::setRandom(uint32_t cnx){
...
void IncConnectionMatrix::setVL2(){
    for (uint32_t src = 0;src<N; src++) {
        connections[src] = new vector<uint32_t>();
...
vector<connection*>* IncConnectionMatrix::getAllConnections(){
    if (conns!=NULL){
...
void IncConnectionMatrix::setStaggeredRandom(Topology* top,uint32_t conns,double local){
...
void IncConnectionMatrix::setStaggeredPermutation(Topology* top,double local){
    int is_dest[N];
...
void IncConnectionMatrix::setManytoMany(uint32_t c){
...
void IncConnectionMatrix::setHotspot(uint32_t hosts_per_hotspot, uint32_t count){
...
void IncConnectionMatrix::setIncastLocalPerm(uint32_t hosts_per_hotspot){
    //int is_dest[N];
    int is_done[N];
...
void IncConnectionMatrix::setHotspotOutcast(uint32_t hosts_per_hotspot, uint32_t count){
    int is_dest[N],is_done[N];
    for (uint32_t i=0;i<N;i++){
        is_dest[i] = 0;
        is_done[i] = 0;
...
void IncConnectionMatrix::setIncast(uint32_t hosts_per_hotspot, uint32_t center){
...
void IncConnectionMatrix::setOutcast(uint32_t src,uint32_t hosts_per_hotspot, uint32_t center){
...
bool IncConnectionMatrix::save(const char * filename){
    //save conns to disk.
...
bool IncConnectionMatrix::save(FILE* f){
    if (!conns)
        getAllConnections();
...
/*
bool IncConnectionMatrix::load(const char * filename){
    //init conns.
...
bool IncConnectionMatrix::load(istream& file){
    ...
    */
    ...
    do {
        ...
        if (dstix != string::npos) {
            ...
            if(destination_part=="#"){
                // This is an external-destination flow, which is not represented by a numerical ID. We use UINT32_MAX as a placeholder to indicate this
                c->dst=UINT32_MAX;
                // We add the source to the job's participant list so that we can assign it a unique destination ID when this flow starts
...
Trigger*
IncConnectionMatrix::getTrigger(triggerid_t id, EventList& eventlist) {
