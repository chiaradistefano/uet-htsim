...
from gen_collectives import gen_array, gen_coll_alltoall
...
TOPOLOGY_PARAM = {
    ...
    "ft":  {"nodes": 16, "strat": "ecmp_host"}
}

# Pattern: "Flow Uec_9_14 413 finished at 23.3024 total bytes 1 flow size 512"
...
def run_simulation(topology, reps, size, other, perA, perB, type):
    ...
    arrayA, arrayB = gen_array(perA, perB, type, nodes , conns)
    _ = gen_coll_alltoall("allgather_bine", "alltoall_isend", nodes , conns, size, "simA.cm", 0 , arrayA, arrayB)
    conn_mat = "simA.cm"
    ...
    for i in range(1, reps + 1):
        logging.info(f"Running only A rep {i}/{reps}: {cmd}")
        _ = gen_coll_alltoall("allgather_bine", "alltoall_isend", nodes , conns, size, "simA_B.cm", start , arrayA, arrayB)
    ...
    if times_A_B:
        ...
        logging.info(
            ...
            f"  Worst: {worst_time_AB:.4f}\n"
            f"  Best:  {best_time_AB:.4f}{RESET}"
        )
    
    return avg_time_A, avg_time_AB, worst_time_AB, best_time_AB
...
if __name__ == "__main__":
    ...
    csv_filename = "simulation_alltoall_results.csv"
    headers = [
    ...
    with open(csv_filename, mode='w', newline='') as csv_file:
        ...
        for t in types:
            for s in size:
                ...
                avgA, avgB, wB, bB = run_simulation(
                    "ft", 5, s, 
