...
# python3 run_tests.py "allgather" "dfp" "1MiB" 1 "-end 100000000 -q 364 -linkspeed 400000 -hop_latency 0.02 -switch_latency 0.25"

# Parameters for specific topologies (if the number of connection is not specified conns = nodes)
TOPOLOGY_PARAM = {
    "dfp": {"nodes": 1332, "conns": 1024, "strat": "fpar"},
...
def main():
    ...
    for topo in topologies:
        ...
        # e.g., "-nodes 1024 -strat ecmp_host"
                    ...
                    try:
                        ...
                        if t is not None:
                            ...
                            if i == args.reps and log:  # FIX: only process logs when logging was actually enabled
                                ...
                                output_filename = log_dir / f"{coll}_fpar_{nodes}n_{conns}c_{size}.csv"
