#!/usr/bin/env python3
"""
Plot allgather simulation-time comparison for Dragonfly+ (log/dfp).
...
recdub, ring) inside each cluster, and simulation time (us) on the y-axis.

Usage:
    python3 plot_dfp_allgather_times.py [--input-dir DIR] [--output FILE]
    python3 plot_dfp_allgather_times.py --colors "#2a78d6,#1baf7a,#eda100,#008300,#4a3aa7"
"""
...
'''"bine": "#2E7D7D", "sparbit": "#6BA5A5",
          "bruck": "#C0492E", "recdub": "#D9836E",
          "ring":  "#9AA3A3"
...
# Matches the text color seaborn's "ticks" style uses in plot_port_util.py.
COLOR_TEXT = ".15"
...
def configure_font(font_family, font_path=None):
    """Register/select the font used for all chart text."""
    if font_path:
...
def plot_data(data, output_path, algo_colors):
    n_algos = len(ALGO_ORDER)
    n_groups = len(GROUP_ORDER)

    bar_width = 0.8 / n_algos
    group_centers = list(range(n_groups))
    label_offsets = compute_label_offsets(data)

    # Font size is large (28pt) by request, so give the chart plenty of room.
    ...
    for i, algo in enumerate(ALGO_ORDER):
        ...
        # Direct labels at the tip of each bar (value in microseconds).
        for group, bar, value in zip(GROUP_ORDER, bars, [data.get(g, {}).get(algo) for g in GROUP_ORDER]):
            if value is None:
                continue
            ax.annotate(
                f"{value:.1f}",
    ...
    ax.set_ylabel("Total simulation time (µs)", fontsize=FONT_SIZE, color=COLOR_TEXT)
...
def main():
    ...
    default_output = os.path.join(default_input_dir, "plot", "allgather_simulation_times.pdf")
    ...
    configure_font(args.font_family, font_path=args.font_path)

    os.makedirs(os.path.dirname(os.path.abspath(args.output)), exist_ok=True)
    plot_data(data, args.output, algo_colors)
